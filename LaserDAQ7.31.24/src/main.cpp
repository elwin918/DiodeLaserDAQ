...
// Calculates the temperature in Farenheit from pin 26 (A12)
...
// Print Values to Serial Monitor:
    if (i != NUMCHANNELS - 1) 
    {
      Serial.print((sptr->avals[i]) * ADCVREF/4096.0, 4); // voltages for the lasers and for set mon and i mon
      
      Serial.print(",");  // print a comma after each value except the last one
    } 
    else
    {
      Serial.println(temp(float (sptr->avals[i])), 4);    // Temperature
    }
  }
  samplecount++;
  bufferindex++;
